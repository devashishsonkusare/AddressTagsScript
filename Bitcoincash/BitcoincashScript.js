...
async function fetchEntityLabel(address, retries = 3) {
    const url = `https://www.oklink.com/api/v5/explorer/address/entity-label?chainShortName=BCH&address=${address}`;

    try {
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                'Ok-Access-Key': '2b42109d-bbca-4cdd-b875-e6e547d133e4'
            }
        });
...
async function createOrUpdateTableAndProcessData() {
    ...
    try {
        ...
        while (hasNextPage) {
            const url = `https://www.oklink.com/api/v5/explorer/transaction/transaction-list?chainShortName=BCH&limit=1000&page=${page}`;
            const response = await fetch(url, {
                method: 'GET',
                headers: {
                    'Ok-Access-Key': '2b42109d-bbca-4cdd-b875-e6e547d133e4'
                ...
                try {
                    ...
                    if (labels.trim() !== '') {
                        const query = tableExists ?
                            'INSERT INTO bitcoincash (addresss, label, risk) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING' :
                            'INSERT INTO bitcoincash (addresss, label, risk) VALUES ($1, $2, $3)';
