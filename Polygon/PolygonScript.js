...
async function fetchEntityLabel(address, retries = 3) {
    const url = `https://www.oklink.com/api/v5/explorer/address/entity-label?chainShortName=POLYGON&address=${address}`;
...
async function createOrUpdateTableAndProcessData() {
    ...
    try {
        ...
        while (hasNextPage) {
            const url = `https://www.oklink.com/api/v5/explorer/transaction/transaction-list?chainShortName=POLYGON&limit=1000&page=${page}`;
            const response = await fetch(url, {
                method: 'GET',
                headers: {
                    'Ok-Access-Key': '05045e6c-c119-44c3-8cd6-f616f4bfa941'
                ...
                try {
                    ...
                    if (labels.trim() !== '') {
                        const query = tableExists ?
                            'INSERT INTO polygon (addresss, label, risk) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING' :
                            'INSERT INTO polygon (addresss, label, risk) VALUES ($1, $2, $3)';
